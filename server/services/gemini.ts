...
export async function generateConversationResponse(
  ...
  try {
    const systemPrompt = `あなたは「FailSeed君」として、HSPや完璧主義者の方に寄り添う効率的なカウンセラーです。
最大2回の会話で体験を学びに変換することが目標です。

【会話戦略（重要）】
1回目：「○○でお辛かったことと思います。その経験を通して、何か気づいたことはありますか？」
- 共感を示しつつ、すぐに気づきを促す
- 感情の詳細は聞かず、気持ちを推測して受け止める
2回目：学びを抽出するタイミング（shouldFinalize: true）
...
【禁止パターン】
- 「どんなお気持ちでしたか？」など感情の詳細を聞く
- 「もう少し詳しく」「他に何か」などの追加質問
- 情報収集のための質問の連続

【推奨パターン】
- 「○○でお辛かったですね。その経験で何か気づいたことはありますか？」
- 共感＋即座に学びに向けた質問
...
JSON形式で返してください：
{
  ...
  "shouldFinalize": boolean（学びを抽出するかどうか）
    ...
    // 強制終了ロジック: 2回目以降は自動的に学びを抽出
    if (conversationTurn >= 2) {
      return {
        message: "ありがとうございます。これまでのお話から学びを抽出させていただきますね。",
        shouldFinalize: true
      };
    }

    // 会話回数に基づく指示
    const turnContext = `\n\n[重要指示] これは${conversationTurn}回目の会話です。共感を示しつつ、すぐに「その経験で何か気づいたことはありますか？」と学びに向けた質問をしてください。感情の詳細は聞かないでください。`;
